...
class WhatsAppSendWizard(models.TransientModel):
    ...
    def _prepare_template_header_media_kwargs(self):
        ...
        raise UserError(_(
            "%s needs a %s header file before sending. Upload a file in this wizard, "
            "set a default Header Media File/URL on the template, or resend using a template "
            "that does not require a media header."
        ) % (self.template_id.display_name, self.template_id.header_type))
