...
class AdmissionTeamMember(models.Model):
    ...
    # mobile field removed in Odoo 19 - use phone instead
    ...
    @api.constrains('admission_team_id', 'user_id', 'active')
    def _constrains_membership(self):
        # In mono membership mode: check admission_team_id / user_id is unique for active
        ...
        existing = self.env['oe.admission.team.member'].search([
            ('admission_team_id', 'in', self.admission_team_id.ids),
            ('user_id', 'in', self.user_id.ids),
            ('active', '=', True)
        ])
        duplicates = self.env['oe.admission.team.member']

        active_records = dict(
            (membership.user_id.id, membership.admission_team_id.id)
            for membership in self if membership.active
        )
        for membership in self:
            potential = existing.filtered(lambda m: m.user_id == membership.user_id and \
                m.admission_team_id == membership.admission_team_id and m.id != membership.id
            )
            ...
            if active_records.get(potential.user_id.id):
                ...
                active_records[potential.user_id.id] = potential.admission_team_id.id

        if duplicates:
            raise exceptions.ValidationError(
                _("You are trying to create duplicate membership(s). We found that %(duplicates)s already exist(s).",
                  duplicates=", ".join("%s (%s)" % (m.user_id.name, m.admission_team_id.name) for m in duplicates)
                 ))

    @api.depends('admission_team_id', 'is_membership_multi', 'user_id')
    @api.depends_context('default_admission_team_id')
    def _compute_user_in_teams_ids(self):
        ...
        if all(m.is_membership_multi for m in self):
            ...
            member_user_ids = self.env['oe.admission.team.member'].search([('id', 'not in', self.ids)]).user_id
        else:
            member_user_ids = self.env['oe.admission.team.member'].search([]).user_id
        for member in self:
            if member_user_ids:
                member.user_in_teams_ids = member_user_ids
            elif member.admission_team_id:
                member.user_in_teams_ids = member.admission_team_id.member_ids
            elif self.env.context.get('default_admission_team_id'):
                member.user_in_teams_ids = self.env['oe.admission.team'].browse(self.env.context['default_admission_team_id']).member_ids
            else:
    ...
    def _compute_user_company_ids(self):
        ...
        for member in self:
            member.user_company_ids = member.admission_team_id.company_id or all_companies
    ...
    def _compute_member_warning(self):
        ...
        if all(m.is_membership_multi for m in self):
            ...
            if not active:
                return
            existing = self.env['oe.admission.team.member'].search([('user_id', 'in', active.user_id.ids)])
            user_mapping = dict.fromkeys(existing.user_id, self.env['oe.admission.team'])
            for membership in existing:
                user_mapping[membership.user_id] |= membership.admission_team_id

            for member in active:
                teams = user_mapping.get(member.user_id, self.env['oe.admission.team'])
                remaining = teams - (member.admission_team_id | member._origin.admission_team_id)
                if remaining:
    ...
    def write(self, values):
        ...
        return super(AdmissionTeamMember, self).write(values)

    def _synchronize_memberships(self, user_team_ids):
        """ Synchronize memberships: archive other memberships.

        :param user_team_ids: list of pairs (user_id, admission_team_id)
        """
        existing = self.search([
            ('active', '=', True),  # explicit search on active only, whatever context
            ('user_id', 'in', [values['user_id'] for values in user_team_ids])
        ])
        user_memberships = dict.fromkeys(existing.user_id.ids, self.env['oe.admission.team.member'])
        for membership in existing:
            user_memberships[membership.user_id.id] += membership

        existing_to_archive = self.env['oe.admission.team.member']
        for values in user_team_ids:
            existing_to_archive += user_memberships.get(values['user_id'], self.env['oe.admission.team.member']).filtered(
                lambda m: m.admission_team_id.id != values['admission_team_id']
            )
